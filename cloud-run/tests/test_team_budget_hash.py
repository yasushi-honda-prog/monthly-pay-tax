...
dashboard/tests/test_lib_team_budget_hash.py と同じ test を持つことで
...
from team_budget_hash import compose_actual_data_hash


# ---- contract cases ----
# (bq_hash, budget_amount, prompt_version, expected_hex)
# expected_hex は cloud-run/team_budget_hash.compose_actual_data_hash の出力で固定。
# dashboard 側 helper も同じ結果を返す必要がある。
CONTRACT_CASES = [
    # データなし (空 bq_hash) + 予算なし
    ("", None, "v1",
...
class TestComposeActualDataHashContract:
    """両側 hash が同入力で同出力であることを保証する contract test"""
