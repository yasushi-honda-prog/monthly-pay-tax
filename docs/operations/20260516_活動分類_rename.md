...
### 4.3 Playwright MCP は使わない

| 項目 | Playwright 自動化 | 判定 |
|------|----------------|------|
| OAuth 同意ダイアログ | 多段ダイアログ、自動化が脆い | × |
| Apps Script エディタ UI | Google が UI を頻繁変更、selector が壊れる | × |
| 実行回数 | 最大 3 回（audit / dry_run / execute） | 自動化コスト過剰 |
| 障害時リカバリ | ブラウザ自動化のデバッグが煩雑 | × |

→ `clasp run` または手動 Run ボタン 1 クリックで十分。
