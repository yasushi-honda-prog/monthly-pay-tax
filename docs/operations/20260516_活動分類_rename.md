...
### 5.5 execute 直前の BQ バックアップ（保険）

本作業は E4 ヘッダー文字列のみの変更で、collector は範囲ベース（`B7:K`）で読むためヘッダー変更が BQ に伝播することはない（§3 参照）。したがって平時の影響経路は薄い。ただし **GAS が想定外の挙動でデータ行（7行目以降）を誤書き換えした場合**、次回バッチが WRITE_TRUNCATE で BQ を上書きしてしまうため、保険として `execute` モード実行の **直前** に `gyomu_reports` / `hojo_reports` の snapshot を取得する。

```bash
# execute 直前に実行（gyomu/hojo を別データセットへ snapshot、90日で自動失効）
DATE=$(date +%Y%m%d)
for T in gyomu_reports hojo_reports; do
  bq query --use_legacy_sql=false \
    "CREATE SNAPSHOT TABLE IF NOT EXISTS \`monthly-pay-tax.pay_reports_backup.${T}_${DATE}\` \
     CLONE \`monthly-pay-tax.pay_reports.${T}\` \
     OPTIONS(expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 90 DAY))"
done
```

万一 rename 作業後の次回バッチで `gyomu_reports` / `hojo_reports` が壊れた場合は、snapshot から復元する。

```bash
bq cp -f monthly-pay-tax:pay_reports_backup.gyomu_reports_YYYYMMDD \
         monthly-pay-tax:pay_reports.gyomu_reports
```

> **注意**: この snapshot は **BQ 側**の保険であり、**Sheets 自体**の保護ではない。Sheets のデータ行が GAS で壊れた場合の復旧は、GAS の旧値ログ（§6）と Google Drive の版履歴で行う。`gyomu_reports` / `hojo_reports` は毎朝 WRITE_TRUNCATE で再生成される=平時は Sheets がソースのため、定常の日次 snapshot 対象（`pay_reports_backup` への自動 snapshot）には含めていない。本ステップは rename という非常時のみの手動取得である。
