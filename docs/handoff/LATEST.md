...
1. **「(仮) 報告入力」採用判断** — admin が試用後、本採用 / 改変 / 廃止を判断
   - 本採用 → ページ内 expander §5「dry-run 解除の手順」(6 ステップ) で復活
   - 廃止 → ページ内 expander §6「不採用時の削除対象」に従って削除
2. **Phase 0 回答受領後**: wam_flag UPDATE → ドラフトラベル除去
3. **#58 外部ツール所在確認後**: 連携フォーマット決定 → CSV調整
4. **#94 コスト監視** — ADR 0004 効果測定（予算アラート + BQ Billing export 設定）

---

> テスト件数は `python3 -m pytest dashboard/tests/ -q && python3 -m pytest cloud-run/tests/ -q` で確認（**380件**、2026-05-13時点）
