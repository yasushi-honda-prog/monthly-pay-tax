...
**更新日**: 2026-04-12（WAM確認ページUX強化 — WAMフィルタ + CSVダウンロード追加）
**フェーズ**: 6完了 + グループ機能 + グループ一括登録・自動同期 + UX改善 + 数値変換リファクタ + 報告入力機能（デプロイ済み）＋ **WAM助成金対応 Phase 1b 完了 + UX強化（フィルタ＆CSV）**
**最新デプロイ**: Collector rev 00023-w7s（立替金シート収集対応）+ Dashboard rev 00223-m56（WAMフィルタ + CSVダウンロード追加）
**Cloud Run設定**: 2026-04-07 `--no-cpu-throttling --max-instances=3` 適用済み（ADR 0004）
**テストスイート**: Dashboard 213 + Cloud Run 42 = **255テスト全PASS**
...
| [#55](https://github.com/yasushi-honda-prog/monthly-pay-tax/issues/55) | **P1** | WAM要件① 領収書の自動生成と振込一括化 (Must) | **着手可能** |
| [#56](https://github.com/yasushi-honda-prog/monthly-pay-tax/issues/56) | **P1** | WAM要件② 振込データ抽出とCSV出力（GMOあおぞら） (Must) | **着手可能** |
| [#57](https://github.com/yasushi-honda-prog/monthly-pay-tax/issues/57) | **P1** | WAM要件③ WAM月別報酬・源泉徴収確認ツール (Must) | **着手可能** |
...
**方針転換（2026-04-12）**: #54 Phase 0 をブロッカーから参考情報に格下げ。ADR_0005（GCP側処理完結）+ プロトタイプ駆動により、実データ・実ツールから仕様を決めて実装 → デモで確認。回答は仕様微調整に活用。

1. **#55-57 (Must) に着手** — 実データ2,250行 + シート構造把握済み、ブロッカーなし
   - #57 WAM月別報酬確認 → 既存VIEW流用で最速着手可能
   - #56 振込CSV出力 → GMOあおぞらフォーマット調査 → 実装
   - #55 領収書自動生成 → receipt_url集約方式で実装
2. **wam_target_projects の wam_flag 更新** — 実データから対象PJを判断（回答待ち不要）
3. **未追跡ファイル整理** — ルートのスクリーンショットPNG 10件 + `.serena/` の .gitignore追加 or 削除
...
> テスト件数は `python3 -m pytest dashboard/tests/ -q` で確認（210件、2026-04-12時点）
