...
**更新日**: 2026-03-28
**フェーズ**: 6完了 + グループ機能 + グループ一括登録・自動同期 + UX改善 + 数値変換リファクタ + 報告入力機能（実装中）
**最新デプロイ**: Collector rev 00020-g6b + Dashboard rev 00166-k42（PR #47/#48/#49 + Infinite extent修正すべてデプロイ済み）
**テストスイート**: 198テスト（全PASS: dashboard 198 + cloud-run 20 = 218合計）
...
### 直近の変更（2026-03-28 現在 — 未コミット・未デプロイ）
...
1. **報告入力機能をコミット・PR作成**
   - 変更済み6ファイル + 新規2ファイルをコミット
   - BQテーブル `app_gyomu_reports` / `app_hojo_reports` を本番BQに作成
   - dashboard デプロイ
