...
```bash
# Dashboard テスト（218テスト）— プロジェクトルートから実行可能
...
1. **import整合性**: 変更ファイルの全import文が既存モジュールに解決可能か確認
2. **型/構造の整合性**: 関数の戻り値を変更した場合、`Grep`で全呼び出し元を確認
...
- BQデータテーブルはすべてSTRING型 → ダッシュボード側で `pd.to_numeric(errors="coerce")` / `clean_numeric_series()` による数値変換が必須
- Streamlit 1.50.0のAltairはVega-Lite v5 specを出力するが、フロントエンドはVega-Lite v6で描画するためコンソール警告が出る（表示に影響なし、Streamlit側のアップデート待ち）
