...
<div class="filter-cards">
    <div class="fc">
        ...
        <ul>
            ...
            <li>「月」で対象月を選択</li>
            <li>「全月」で年間表示</li>
            <li><strong>期間指定モード</strong>: 業務報告一覧・WAM業務報告は<br>サイドバーの「月」セレクタで「期間指定」を選ぶと<br>年跨ぎ範囲（開始月〜終了月）で絞り込み可能</li>
            <li>デフォルト: 最新年・全月</li>
        </ul>
    </div>
    <div class="fc">
...
st.markdown(
    ...
    "WAM業務報告は業務分類が「**（WAM）**」または「**(WAM)**」始まり"
    "（全角・半角の括弧どちらも、先頭の空白も自動で吸収）の行のみを対象に同じUIで操作できます。"
)
...
<div class="filter-cards">
    <div class="fc">
        <h4>📅 期間指定モード</h4>
        <ul>
            <li>サイドバーの「<strong>月</strong>」セレクタで「<strong>期間指定</strong>」を選択</li>
            <li>「開始月」「終了月」を <code>YYYY年M月</code> 形式で指定</li>
    ...
    <div class="fc">
        <h4>🔗 依存型ドロップダウン</h4>
        <ul>
            <li>「隊（活動）分類」を選ぶと、その分類に紐づく「業務分類」「スポンサー」だけが選択肢に絞り込まれる</li>
    ...
    <div class="fc">
        <h4>🔎 キーワード検索 + 検索対象選択</h4>
        <ul>
            <li>テキスト入力欄でキーワード検索</li>
            <li>「<strong>検索対象</strong>」multiselect で対象カラムを限定（空 = 全カラム横断 / 選択あり = 選択カラムのみ OR 検索）</li>
...
<div class="tip info">
    ...
    <div class="tip-c">
        旧表記「メンバー数 N」は「全在籍数 名のうち N 名」と「サイドバー選択 N 名のうち N 名」を見分けられない問題がありました。<br>
        新表記では <strong>分母</strong> がフィルター条件に応じて自動切替されます:<br>
        ・サイドバーで全選択 → 分母 = <strong>全在籍数</strong>（タダメンMマスタの在籍メンバー総数、時期により変動）<br>
...
with st.expander("「報告者数 N」と「全在籍 M 名」のどっちが正しい？"):
    st.markdown("""
    どちらも正しい数字ですが、**意味が違います**:

    - **報告者数 N**: 期間 + フィルター適用後に **1 件以上報告した人** の実数
    - **全在籍 M 名**: 在籍メンバー（タダメンMマスタ）の総数。サイドバーで全選択した場合の分母（時期により変動）

    新しい「報告者数 X / Y 名」表記は、**サイドバーで何人選んでいるか** に応じて分母が変わります:

    - サイドバーで全選択 → 「N / 全在籍数 名」（全在籍のうち報告者 N）
...
with st.expander("年跨ぎで絞り込みたい（例: 2025-11〜2026-06）"):
    ...
    1. サイドバーの「**月**」セレクタで **「期間指定」** を選択
    2. 出現する「開始月」「終了月」のセレクタで `YYYY年M月` 形式で範囲を指定
...
with st.expander("業務報告一覧の検索は何を対象にしている？"):
    st.markdown("""
    「**検索対象**」multiselect で切替可能です:

    - **空のまま**: 表示中の **全カラムを横断検索**（隊（活動）分類・業務分類・スポンサー・内容・名前など）
    - **カラム選択あり**: 選択したカラムのみで検索（複数選択 OR）

    例えば「内容」だけ選べば、隊（活動）分類やスポンサー名にヒットせず内容欄のみでの検索になります。
    """)

with st.expander("「内容」列が読みにくい / 長文が切れる"):
    st.markdown("""
    業務報告一覧の「内容」列は **22 文字目安で自動改行** されるようになっています。
